...
export class PostController {
    ...
    static async getPostByUser(req, res, next) {
        ...
        try {
           const postCount = await Post.countDocuments({user_id: userId});
            ...
            const posts: any = await Post.find({user_id: userId}, {__v:0, user_id:0}).populate('comments').skip((perPage * page) - perPage).limit(perPage);
            res.json({
                post: posts,
                pageToken: pageToken,
    ...
    static async getAllPosts(req, res, next) {
        ...
        try {
            const postCount = await Post.estimatedDocumentCount();
             ...
             const posts: any = await Post.find({}, {__v:0, user_id:0}).populate('comments').skip((perPage * page) - perPage).limit(perPage);
             res.json({
                 post: posts,
                 pageToken: pageToken,
                 totalPages: totalPages,
    ...
    static async deletePost(req, res, next) {
        const post = req.post;
        try {
            await post.deleteOne(); // Use deleteOne to trigger the hook
            res.send(post);
        } catch (e) {
            next(e);
        }
